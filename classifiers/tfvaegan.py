...
import numpy as np
from ..dataloaders import tfvaegan as util
import copy
...
class CLASSIFIER:
    ...
    def __init__(self, _train_X, _train_Y, data_loader, _nclass, _cuda, _lr=0.001, _beta1=0.5, _nepoch=20, _batch_size=100, generalized=True, netDec=None, dec_size=4096, dec_hidden_size=4096):
        ...
        if self.netDec:
            ...
            self.model = LINEAR_LOGSOFTMAX_CLASSIFIER(
                self.input_dim, self.nclass)
    ...
    def fit_zsl(self):
        ...
        for epoch in range(self.nepoch):
            ...
            acc = self.val(self.test_unseen_feature,
    ...
    def compute_per_class_acc_gzsl(self, test_label, predicted_label, target_classes):
        ...
        for i in target_classes:
            idx = (test_label == i)
            acc_per_class += torch.sum(test_label[idx]
    ...
    def val(self, test_X, test_label, target_classes):
        ...
        acc = self.compute_per_class_acc(util.map_label(
    ...
    def compute_per_class_acc(self, test_label, predicted_label, nclass):
        ...
        for i in range(nclass):
            ...
            acc_per_class[i] = torch.sum(
                test_label[idx] == predicted_label[idx]) / torch.sum(idx)
    ...
    def compute_dec_out(self, test_X, new_size):
        ...
        for i in range(0, ntest, self.batch_size):
            ...
            new_test_X[start:end] = torch.cat(
                [inputX, feat1, feat2], dim=1).data.cpu()
            start = end
        return new_test_X
